...
import { Users, Package, CalendarDays, Clock, MapPin, ChevronLeft, ChevronRight, Play, CheckCircle } from "lucide-react";
...
const DashboardPage = () => {
  ...
  const { clientes, stock, tareas, updateTareaEstado } = useData();
  const { user } = useAuth();
  ...
  return (
    <div className="space-y-8">
      <div className="text-center">
        <h1 className="text-2xl font-semibold tracking-tight">{user?.nombre || "Dashboard"}</h1>
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
        {[
          ...
          { label: "Productos en stock", value: stock.reduce((a, s) => a + s.cantidad, 0), icon: Package },
          { label: "Tareas hoy", value: tareas.filter((t) => t.fecha === new Date().toISOString().split("T")[0]).length, icon: CalendarDays },
      ...
      <Card className="border shadow-sm">
        <CardHeader className="flex-row items-center justify-between pb-4">
          ...
          <div className="flex items-center gap-2">
            <button onClick={() => setDayOffset((p) => p - 1)} className="rounded-lg p-1.5 text-muted-foreground transition-colors hover:bg-secondary hover:text-foreground">
          ...
          <AnimatePresence mode="wait">
            <motion.div key={dateStr} initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} transition={{ duration: 0.2 }} className="space-y-3">
              {tareasDelDia.length === 0 ? (
                <p className="py-8 text-center text-sm text-muted-foreground">No hay tareas programadas para este día</p>
              ) : (
                tareasDelDia.map((tarea) => (
                  <div key={tarea.id} className="flex items-start gap-4 rounded-lg border p-4 transition-colors hover:bg-muted/50">
